...
def getPicFromHtml(html, name):
    ...
    for x in range(count - 1, -1, -1):
        ...
        try:
            ...
            alt = ''
        if 'logo' in src.lower() or 'logo' in alt.lower() or 'footer' in src.lower() or 'footer' in alt.lower() or src[
                                                                                                                   -3:] == 'svg':
            del imgs[x]
    try:
        if imgs[0]:
...
def getTrainFeas():
    ...
    f_fea.write(
        'userid,label,idx0,idx1,idx2,idx3,idx4,idx5,idx6,idx7,idx8,idx9,title_name,url_edu,url_name,url_profile,'
    ...
    if not hpidxis0:
        ...
        for i, line in enumerate(uid_hpidx_lines):
            ...
            user_dic['homepage'] = getHomepageWithSearchpageAndHpidx(user_dic['id'], user_dic['search_results_page'],
                                                                     user_dic['hpidx'])
            print("---homepage " + user_dic['homepage'])
        ...
        for user_dic in tqdm(userlist_vali):
            user_dic['hpidx'] = 0
            #  print("----url----"+user_dic['search_results_page']+" ------ "+user_dic['name']+" -------- "+user_dic['org']+" -------- "+user_dic['id'])
            user_dic['homepage'] = getHomepageWithSearchpageAndHpidx(user_dic['id'], user_dic['search_results_page'],
                                                                     user_dic['hpidx'])
            print("---homepage " + user_dic['homepage'])
            html = getHomepageHTMLWithUseridAndURLAndHpidx(user_dic['id'], user_dic['homepage'], user_dic['hpidx'])
